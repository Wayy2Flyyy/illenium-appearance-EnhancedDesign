...
const TattooTile = styled.button<{ active: boolean; applied: boolean }>`
  ...
  text-align: left;
  border: 1px solid ${({ active, applied }) => (active || applied ? 'rgba(var(--w2f-accent), 0.72)' : 'var(--w2f-border)')};
  border-radius: 10px;
  background: ${({ active, applied }) =>
    active || applied
      ? 'linear-gradient(135deg, rgba(var(--w2f-accent), 0.48), rgba(10, 10, 12, 0.84))'
  ...
  &:hover {
    color: #fff;
    border-color: rgba(var(--w2f-accent), 0.82);
