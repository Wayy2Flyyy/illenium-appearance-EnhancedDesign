...
import AccentControl, { ACCENT_SWATCHES } from './AccentControl';

import { Wrapper, Container, AppearanceBrandCredit } from './styles';

const UI_OPACITY_STORAGE_KEY = 'w2f.illenium.uiOpacity.v1';
const UI_ACCENT_STORAGE_KEY = 'w2f.illenium.accent.v1';

const DEFAULT_ACCENT = ACCENT_SWATCHES[0].rgb;

const loadStoredOpacity = (): number => {
  if (typeof window === 'undefined' || !window.localStorage) return 1;
  const raw = parseFloat(window.localStorage.getItem(UI_OPACITY_STORAGE_KEY) ?? '');
  if (!Number.isFinite(raw)) return 1;
  // Clamp to the slider's usable range so the menu can never load fully hidden.
  return Math.min(1, Math.max(0.2, raw));
};

const loadStoredAccent = (): string => {
  if (typeof window === 'undefined' || !window.localStorage) return DEFAULT_ACCENT;
  const raw = window.localStorage.getItem(UI_ACCENT_STORAGE_KEY);
  // Only honour one of the known swatches; ignore anything stale/tampered.
  return ACCENT_SWATCHES.some(s => s.rgb === raw) ? (raw as string) : DEFAULT_ACCENT;
};

const applyAccent = (rgb: string) => {
  if (typeof document !== 'undefined') {
    document.documentElement.style.setProperty('--w2f-accent', rgb);
  }
};
...
const Appearance = () => {
  ...
  const [uiOpacity, setUiOpacity] = useState<number>(() => loadStoredOpacity());

  const handleOpacityChange = useCallback((value: number) => {
    setUiOpacity(value);
    if (typeof window !== 'undefined' && window.localStorage) {
      try {
        window.localStorage.setItem(UI_OPACITY_STORAGE_KEY, String(value));
      } catch {
        /* storage full / disabled — opacity simply won't persist this session */
      }
    }
  }, []);

  const [accent, setAccent] = useState<string>(() => loadStoredAccent());

  // Apply the stored accent to the CSS var before/at first paint so the menu
  // never flashes the default red when a different colour was chosen.
  useEffect(() => {
    applyAccent(accent);
  }, []);

  const handleAccentChange = useCallback((rgb: string) => {
    setAccent(rgb);
    applyAccent(rgb);
    if (typeof window !== 'undefined' && window.localStorage) {
      try {
        window.localStorage.setItem(UI_ACCENT_STORAGE_KEY, rgb);
      } catch {
        /* storage full / disabled — accent simply won't persist this session */
      }
    }
  }, []);
        ...
        ({ item, key, props: style }) =>
          item && (
            <animated.div key={key} style={style}>
              <OpacityControl value={uiOpacity} onChange={handleOpacityChange} />
              <AccentControl value={accent} onChange={handleAccentChange} />
              <Wrapper style={{ opacity: uiOpacity }}>
