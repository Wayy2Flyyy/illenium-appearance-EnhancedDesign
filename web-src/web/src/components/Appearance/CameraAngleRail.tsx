...
const AngleBtn = styled.button<{ $active: boolean }>`
  ...
  border: 1px solid
    ${({ $active }) => ($active ? 'rgba(var(--w2f-accent), 0.75)' : 'rgba(255, 255, 255, 0.14)')};
  background: ${({ $active }) =>
    $active ? 'rgba(var(--w2f-accent), 0.14)' : 'transparent'};
  box-shadow: ${({ $active }) =>
    $active ? '0 0 14px rgba(var(--w2f-accent), 0.2)' : 'none'};
  ...
  &:hover {
    ...
    background: ${({ $active }) =>
      $active ? 'rgba(var(--w2f-accent), 0.2)' : 'rgba(255, 255, 255, 0.06)'};
  }
`;
