...
const TextureSlider = styled.input.attrs({ type: 'range' })`
  ...
  &::-webkit-slider-thumb {
    ...
    height: 18px;
    border-radius: 999px;
    background: rgba(${props => props.theme.primaryBackground || '239, 68, 68'}, 1);
    border: 2px solid rgba(255, 255, 255, 0.92);
    box-shadow: 0 0 16px rgba(${props => props.theme.primaryBackground || '239, 68, 68'}, 0.6);
    cursor: pointer;
    margin-top: -7px;
  }
...
const SlotDialogButton = styled.button<{ tone?: 'primary' | 'ghost' | 'danger' }>`
  ...
  background: ${({ tone }) => (tone === 'primary' ? 'rgba(239, 68, 68, 0.18)' : 'transparent')};
  color: #fff;
  font-size: 10px;
  font-weight: 900;
  letter-spacing: 0.14em;
